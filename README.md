...
Đã hoàn tất `Phase 0: Scope And Decisions`, `Phase 1: Core Design`, `Phase 2: Scaffold`, và năm bước đầu của `Phase 3: Ingestion` là connect path vào Photos framework, Photos permission flow, asset enumeration, iCloud-backed original access path, và in-memory thumbnail/video representation extraction theo [docs/mvp-checklist.md](/Users/hoaiduc/Documents/VectorDB Image/docs/mvp-checklist.md).
...
Tiếp tục `Phase 3: Ingestion`: chuyển sang repository interface cho local DB và vector layer sau khi thumbnail `224x224` và video poster-frame extraction in-memory đã có CLI verify path riêng.
...
- index pipeline tối thiểu và local persistence layer vẫn tiếp tục được implement ở các step sau của `Phase 3`
