...
Đã hoàn tất `Phase 0: Scope And Decisions`, `Phase 1: Core Design`, `Phase 2: Scaffold`, và sáu bước đầu của `Phase 3: Ingestion` là connect path vào Photos framework, Photos permission flow, asset enumeration, iCloud-backed original access path, in-memory thumbnail/video representation extraction, và repository interface cho local catalog cùng vector layer theo [docs/mvp-checklist.md](/Users/hoaiduc/Documents/VectorDB Image/docs/mvp-checklist.md).
...
Tiếp tục `Phase 3: Ingestion`: chuyển sang index pipeline tối thiểu `scan -> extract -> normalize -> persist` sau khi repository interface cho local catalog và vector layer đã hoạt động.
...
- index pipeline tối thiểu vẫn tiếp tục được implement ở step kế tiếp của `Phase 3`; local persistence layer giờ đã có repository interface và local backend đầu tiên
