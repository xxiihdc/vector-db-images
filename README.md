...
Tiếp tục `Phase 4: Search And Retrieval`: ưu tiên bước ghi asset matches thật vào Photos và command `search "..."` sau khi album output flow đã có đủ ordered write-set cho `AI Search Results`.
...
- runtime hiện đã có album service và Python bridge command để tạo hoặc tìm lại album `AI Search Results`; bước add asset matches vào album vẫn là task riêng tiếp theo
- album output flow hiện đã normalize retrieval results thành ordered unique `local_identifier` write-set, giữ `album_write_mode`, và track unresolved rows trước khi gọi native album mutation
