...
Tiếp tục `Phase 4: Search And Retrieval`: ưu tiên local semantic search và album output sau khi provider abstraction đầu tiên đã được nối vào index pipeline.
...
- khi `index --no-cache` chạy refresh thật, flow nối scan + extraction + normalize + persist vào JSON-backed local stores rồi gọi embedding provider abstraction để sinh semantic vector hoàn toàn in-memory
